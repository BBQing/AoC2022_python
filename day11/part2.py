...
class MonkeyBusiness:
    ...
    def limit(self):
        if self._limit is None:
            self._limit = reduce(mul, (monkey.test_value for monkey in self))
        return self._limit

    @property
    def activity(self):
        return [monkey.counter for monkey in self]
...
class Monkey:
    ...
    def inspect(self):
        self.counter += 1
        item = self.items.pop()
        item = self.operation(item) % self.game.limit
        return item
...
if __name__ == '__main__':
    ...
    print(monkey_business.limit)
    for i in range(10000):
        print(f"Round: {i}")
        for monkey in monkey_business:
            monkey.play()

    print(monkey_business.activity)
